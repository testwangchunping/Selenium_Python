...
from WaiQin.src.service.wq_import import WqImport
...
class ReadOldImport(object):
    ...
    def read_old_import(self):
        ...
        for i in range(rowNum):
            ...
            if i % 2 == 0:
                ...
                for j in range(num):
                    ...
                    # 导入
                    WqImport(self.driver, self.logger, get_module_name).test_old_import()
        ...
        if i % 2==1 and i==count:
            ...
            WqImport(self.driver, self.logger, last_module_name).test_old_import()
            # 页面刷新到首页
            self.driver.get(self.readConfig.f5_url)
        if i==0 and i==count:
