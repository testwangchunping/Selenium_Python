...
from WaiQin.src.wq_import_export.service.export import Export
...
class JugeOldExport(object):
    ...
    def juge_old_export(self):
        ...
        if iee.is_element_exist(By.ID, self.export_id):
            Export(self.driver, self.logger).old_export(self.module_name)
        else:
            pass
        if iee.is_element_exist(By.ID, self.export_photo_id):
